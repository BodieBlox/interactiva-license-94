...
import { Search, RefreshCw, KeyRound, CheckCircle, XCircle, Clock, CalendarClock, ArrowUp } from 'lucide-react';
...
export const UpgradeRequests = () => {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://orgid-f590b-default-rtdb.firebaseio.com/licenseRequests/${selectedRequest.id}.json`, {
        method: 'PATCH',
        body: JSON.stringify({
      ...
      toast({
        title: "Success",
        description: `Request from ${selectedRequest.username} has been ${actionType === 'approve' ? 'approved' : 'rejected'}`,
      });
  ...
  const renderRequestTypeBadge = (type?: string) => {
    ...
    switch (type) {
      case 'extension':
        return <Badge variant="outline" className="border-amber-500 text-amber-500">Extension</Badge>;
      case 'upgrade':
        return <Badge variant="outline" className="border-purple-500 text-purple-500">Tier Upgrade</Badge>;
      default:
        return null;
    }
  };
      ...
      <Card variant="glass" className="mb-6 p-6">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className="w-full sm:w-auto">
            ...
            <p className="text-sm text-muted-foreground">Manage license extension and tier upgrade requests</p>
          </div>
                ...
                <TableRow>
                  <TableCell colSpan={7} className="h-24 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <ArrowUp className="h-8 w-8 text-muted-foreground/30" />
        ...
        <DialogContent className="sm:max-w-md glass-panel border-0 animate-scale-in">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              <span>
                {actionType === 'approve' ? 'Approve' : 'Reject'} {selectedRequest?.requestType === 'extension' ? 'License Extension' : 'Tier Upgrade'} Request
              </span>
            </DialogTitle>
            <DialogDescription>
              {actionType === 'approve'
                ? `This will approve the ${selectedRequest?.requestType === 'extension' ? 'license extension' : 'tier upgrade'} request from ${selectedRequest?.username}.`
                : `This will reject the ${selectedRequest?.requestType === 'extension' ? 'license extension' : 'tier upgrade'} request from ${selectedRequest?.username}.`
              }
            </DialogDescription>
          </DialogHeader>
